...
.controller('ProfileCtrl', ['$rootScope', '$scope', '$routeParams', '$http', 'Auth', function($rootScope, $scope, $routeParams, $http, Auth) {
    ...
    function fetchFeeds() {
      $http({method: 'GET', url: '/api/feeds/' + username}).
      success(function(data, status, headers, config) {
        if (data.length > 0) {
          $scope.feeds = data;
        }
    ...
    $scope.submitNewFeed = function() {
      $http.post('/api/feeds/' + username + '/' + this.feedName, {private: this.private}).
        success(function(data, status, headers, config) {
    ...
    $scope.deleteFeed = function(feed) {
      $http.delete('/api/feeds/' + username + '/' + feed.name).
        success(function(data, status, headers, config) {
          console.log('success');
...
.controller('FeedCtrl', ['$rootScope', '$scope', '$routeParams', '$http', function($rootScope, $scope, $routeParams, $http) {
    ...
    $http({method: 'GET', url: '/api/feeds/' + username + '/' + feedName}).
    success(function(data, status, headers, config) {
    ...
    $http({method: 'GET', url: '/api/feeds/' + username + '/' + feedName + '/subscription'}).
    success(function(data, status, headers, config) {
    ...
    $scope.addPost = function(data) {
      return $http.post('/api/feeds/' + username + '/' + feedName + '/main', {postText: this.postText}).
      success(function(data, status, headers, config) {
    ...
    $scope.updateComment = function(newComment, id, index, commentIndex) {
      ...
      success(function(data, status, headers, config) {
        console.log(JSON.stringify($scope.posts[index].comments))
